...
import com.example.gudumap.sensors.DeadReckoningEngine as BackwardCompatibleEngine
...
class DeadReckoningEngineIntegrationTest {
    ...
    @Test
    fun testBackwardCompatibilityAdapter() {
        // Test that com.example.gudumap.sensors.DeadReckoningEngine works exactly as NavigationScreen expects
        val legacyEngine = BackwardCompatibleEngine()
        legacyEngine.initialize(latitude = 51.7520, longitude = -1.2577, speedMps = 10f, bearingDeg = 45f)
        var drState = legacyEngine.getState()

        assertTrue(drState.isInitialized)
        assertEquals(51.7520, drState.latitude, 1e-6)
        assertEquals(-1.2577, drState.longitude, 1e-6)

        // Update with world-frame accelerations as NavigationScreen.kt does
        drState = legacyEngine.update(accelerationNorth = 1.0f, accelerationEast = 0.5f, heading = 45f)
        assertNotNull(drState)
        assertTrue("Speed should be non-negative", drState.speed >= 0f)

        // Correct with GNSS
        legacyEngine.correctWithGnss(latitude = 51.7522, longitude = -1.2575)
        val stateAfterGnss = legacyEngine.getState()
        assertEquals(51.7522, stateAfterGnss.latitude, 1e-6)

        // Reset
        legacyEngine.reset()
        val resetState = legacyEngine.getState()
        assertFalse(resetState.isInitialized)
        assertEquals(0.0, resetState.latitude, 1e-6)
    }
