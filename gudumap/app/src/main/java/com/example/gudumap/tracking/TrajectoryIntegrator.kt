...
class TrajectoryIntegrator(
     ...
     * @param isReanchoring When true, prevents position shift from counting towards travelled distance.
     */
    fun addPoint(point: TrajectoryPoint, isReanchoring: Boolean = false) {
        synchronized(lock) {
            if (points.isNotEmpty()) {
                ...
                if (!point.isStationary && !isReanchoring && dist > 0.0) {
    ...
    fun getBlackoutStartPoint(): TrajectoryPoint? = synchronized(lock) { blackoutStartPoint }

    fun getBlackoutStartDistance(): Double = synchronized(lock) { blackoutStartDistance }

    fun getBlackoutDistance(): Double = synchronized(lock) { max(0.0, totalDistanceTravelled - blackoutStartDistance) }
